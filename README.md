...
The safety zone is a virtual area around an AGV/AMR. The Safety Bubble Detectors are used to detect the presence of any object inside this zone and prevent the AGV/AMR from colliding with the object.

The **ADI 3DToF Safety Bubble Detector** is developed as a ROS application running on the ADI’s *EVAL-ADTF3175D-NXZ* Time-of-Flight platform. The Safety Bubble Detection algorithm is highly optimized to run at 30FPS on the *EVAL-ADTF3175D-NXZ* platform. The node uses [*ADI ToF SDK*](https://github.com/analogdevicesinc/ToF/) APIs to capture the frames from the sensor. The algorithm is run on the captured images and the output is published as ROS topics. The Node publishes the detection flag and the output visualization image as the topics. The Depth and IR images are also published as ROS topics.
...
```bash
roslaunch adi_3dtof_safety_bubble_detector adi_3dtof_safety_bubble_detector_single_camera.launch arg_input_sensor_mode:=0
```
...
```bash
cd ~/catkin_ws/src/adi_3dtof_safety_bubble_detector
git pull
cd ~/catkin_ws
...
### WSL as host
> [!note]       
> If you are using WSL as the Host machine, then setting the Host as ROS Master does not work. In this case, you must unset the ROS master on the device.
...
catkin build -DCMAKE_BUILD_TYPE=Release -DSENSOR_CONNECTED=False
```
...
1. Go to the installation directory of the ADI 3DToF ADTF31xx application `~/Analog Devices/ADI3DToFSafetyBubbleDetector-Rel1.2.0`
2. Run the `get_videos.sh` script which will download the `adi_3dtof_input_video_files.zip` file in the current directory.
3. Unzip it and copy the directory to `~/catkin_ws/src/adi_3dtof_input_video_files`.
4. Update the input file argument `arg_in_file_name` in the launch file `adi_3dtof_safety_bubble_detector_single_camera.launch` as per the above file path.
...
# Launch the package
roslaunch adi_3dtof_safety_bubble_detector adi_3dtof_safety_bubble_detector_single_camera.launch arg_input_sensor_mode:=2
```
...
```bash
roslaunch adi_3dtof_safety_bubble_detector adi_3dtof_safety_bubble_detector_single_camera arg_input_sensor_mode:=3 arg_input_sensor_ip:=10.43.0.1
```
> [!note]
...
|-----------------------------------------|---------------|------------------------------|-------------------------------------------------|
| **cam_base_frame**                      | String        | "adi_camera_link"            | Name of camera Link                             |
| **cam_base_frame_optical**              | String        | "optical_camera_link"        | Name of optical camera Link                     |
| **virtual_camera_base_frame**           | String        | "virtual_camera_link"        | Name of virtual camera Link                     |
...
| **arg_input_mode**                      | int           | 0                            | Input mode, _0: Real Time Sensor, 2: Rosbag bin, 3: Network_  |
| **arg_output_mode**                     | int           | 0                            | Output mode, _0: No output files written, 1: avi and csv output files are written_  |
| **arg_input_file_name_or_ros_topic_prefix_name** | String | "no name"                  | Input filename: Applicable only if the input mode is 2 Represents input file name |
| **arg_enable_ransac_floor_detection**       | int        | 1                           | Enable option for RANSAC floor detection, _0: disable, 1: enable_   |
| **arg_enable_depth_ab_compression**         | int        | 0                           | Enable option to publish depth and IR compressed images, _0: disable, 1: enable_|
| **arg_enable_output_image_compression**     | int        | 0                           | Enable option to publish compressed output image, _0: disable, 1: enable_   |
| **arg_safety_bubble_detection_sensitivity** | int        | 10                          | Number of connected pixels to trigger object detection       |
| **arg_enable_floor_paint**                  | int        | 0                           | Enable option to visualize floor paint, _0: disable, 1: enable_   |
| **arg_enable_safety_bubble_zone_visualization** | int   | 0                            | Enable option to visualize safety bubble zone, _0: disable, 1: enable_ |
...
| **arg_confidence_threshold**                | int        | 10                          | Confidence threshold for the sensor. Default is 10 for CR/DV series, 25 for AM series.|
| **arg_config_file_name_of_tof_sdk**         | String     | "config/config_adsd5300_adsd3100.json"| Configuration file name of ToF SDK. Varies based on Eval Board series. |
| **arg_camera_mode**                         | int        | 3                           | Camera Mode. Refer to launch file for usage   |
