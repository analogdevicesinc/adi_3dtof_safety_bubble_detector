...
>       ```bash
>       $ scp Fw_Update_5.2.5.bin analog@10.42.0.1:/home/analog/Workspace
>          Username: analog 
>          Password: analog
>       ```    
...
## Getting Started with the Software package
### Image Setup
...
1. On the EVAL-ADTF3175D-NXZ device, the ROS Master is set to the IP address of the Host machine, hence it is required to run `roscore` on the Host machine (*applicable only to Linux host*).  
    ...
    To see the depth and IR images on the Host machine, simply open the RVIZ and add ```/cam1/depth_image```, ```/cam1/ab_image``` and ```/cam1/out_image``` topics to visualize the images

2. Publishing compressed output image  
...
| **/object_detected**            | Boolean to indicate object detection                                                                |
| **/camera_info**                | Camera information                                                                                  |
| **/depth_image/compressedDepth**| 512x512 16-bit Depth image compressed with RVL compression (if RVL image-compression is enabled)     |
| **/ab_image/compressedDepth**   | 512x512 16-bit IR image compressed with RVL compression (if RVL image-compression is enabled)        |
| **/out_image/compressed**       | 512x512 8-bit output image compressed with RVL compression (if RVL image-compression is enabled)     |
...
| **/out_image/compressed** | Subscribes output image from adi_3dtof_safety_bubble_detector node |
