...
public:
   ...
   */
  FloorPlaneDetection(
    int image_width = 512, int image_height = 512, float ransac_distance_threshold_mtr = 0.025f,
  ...
  int preProcessPointCloud(
    short * input_point_cloud, float * modified_point_cloud, bool * nearby_objects_flag);
  bool detectFloorUsingEnhancedRANSAC(
    unsigned short * output_depth_image, short * output_point_cloud, FilterFlag filter_flag,
    unsigned char * floor_mask, unsigned char * compressed_floor_mask, short * input_point_cloud,
    float * modified_point_cloud, int modified_point_cloud_size, bool nearby_objects_flag);
...
private:
  ...
  void postProcessPointCloud(
    pcl::ModelCoefficients & model_coefficients, unsigned short * output_depth_image,
    short * output_point_cloud, unsigned char * floor_mask, unsigned char * compressed_floor_mask);
};
