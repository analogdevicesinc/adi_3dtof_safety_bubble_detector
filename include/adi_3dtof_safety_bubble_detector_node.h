...
public:
  ...
  ADI3DToFSafetyBubbleDetector() : Node("adi_3dtof_safety_bubble_detector_node")
  {
    ...
    // Note: Uses the maximum radius across all enabled zones (zones are now independent from origin)
    {
      auto& zones_init = multi_zone_config_.getZones();
      float max_zone_radius = 1.0f;  // Default 1.0m
      for (const auto& z : zones_init) {
        if (z.enabled) {
          max_zone_radius = std::max(max_zone_radius, z.radius_mtr);
        }
      }
    ...
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    if (zone_params_changed) {
      RCLCPP_INFO(this->get_logger(), 
                  "Zone parameters updated - Z1(r=%.2f,x=%.2f) Z2(r=%.2f,x=%.2f) Z3(r=%.2f,x=%.2f)",
