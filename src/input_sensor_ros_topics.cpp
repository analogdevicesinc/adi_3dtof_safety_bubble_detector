                                     ...
                                     std::string /*config_file_name*/, std::string input_sensor_ip)
{
  ...
  ab_image_subscriber_.subscribe(nh, ab_image_topic_name, 5);
  ...
  sync_depth_and_ir.connectInput(depth_image_subscriber_, ab_image_subscriber_);
...
void InputSensorRosTopic::syncDepthandIr(const sensor_msgs::ImageConstPtr& depth_image,
                                         const sensor_msgs::ImageConstPtr& ab_image)
{
...
bool InputSensorRosTopic::readNextFrame(unsigned short* depth_frame, unsigned short* ab_frame)
{
  ...
  if (!time_out_flag)
  {
    ...
    memcpy(ab_frame, new_frame->ab_image_, sizeof(new_frame->ab_image_[0]) * frame_width_ * frame_height_);
    // Extract timestamp
