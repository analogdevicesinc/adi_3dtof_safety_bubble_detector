...
static void ReadProfile(const char* prof_file_name)
{
  ...
  while (!is_stream_end(prof_file))
  {
    ...
    stream_read((char*)&nanosecs_timestamp, sizeof(nanosecs_timestamp), NUM_PERF_COUNTERS, prof_file);
    ...
    if (profiles[profile_handle].start_end == 1)
    {
      ...
      for (i = 0; i < NUM_PERF_COUNTERS; i++)
      {
        if (i == 0)
        {
          millisecs_spent = millisecs[FOCUS_COUNTER] - profiles[profile_handle].millisecs[FOCUS_COUNTER];
...
static void CloseProfileParsing(void)
{
  ...
  for (handle = 0; handle <= profile_count; handle++)
  {
    ...
    {
      profiles[handle].avg_millisecs[0] = profiles[handle].total_millisecs[0] / profiles[handle].num_calls;
#ifndef DETAILED_PERF_ANALYSIS
      ...
      {
#ifndef DETAILED_PERF_ANALYSIS
        sprintf(out_msg, "%f(%2.2f)", profiles[handle].maximum_millisecs[i],
                (float)profiles[handle].maximum_millisecs[i] * 100 / profiles[handle].maximum_millisecs[0]);
        fprintf(stdout, "%-22s", out_msg);
#else
        profiles[handle].event_millisecs[i + profile_event_start - 1] = profiles[handle].maximum_millisecs[i];
#endif
      }
...
void dumpFunctionParams(int ID)
{
  ...
  do
  {
    *pprof_buff++ = ID;
    ...
    // ros::Time curr_frame_timestamp_after;
