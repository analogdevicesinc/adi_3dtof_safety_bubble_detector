 ...
 */
void InputSensorFile::openSensor(std::string sensor_name, int input_image_width, int input_image_height,
...
void InputSensorFile::configureSensor(std::string /*frame_type*/)
{
  ...
  if (in_file_.is_open())
  {
    ...
    int num_bytes_in_combo_frame = (frame_width_ * frame_height_ * bytes_per_pixel_) * 5;  // *2 for depth, ir and x y z
 ...
 */
bool InputSensorFile::readNextFrame(unsigned short* scaled_depth_frame, unsigned short* scaled_ir_frame)
{
  // Read next frame
  int num_samples_in_frame = (frame_width_ * frame_height_);
  int num_bytes_in_combo_frame = (num_samples_in_frame * bytes_per_pixel_) * 5;  // 5 for depth, ir and x y z
 ...
 *
 */
bool InputSensorFile::getFrameTimestamp(ros::Time* timestamp)
{
