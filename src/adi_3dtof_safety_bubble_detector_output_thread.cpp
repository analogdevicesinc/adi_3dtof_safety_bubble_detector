 ...
 *
 */
void ADI3DToFSafetyBubbleDetector::processOutputAbort()
{
...
void ADI3DToFSafetyBubbleDetector::processOutput()
{
  ...
  while ((!process_output_thread_abort_) || (output_queue_size > 0))
  {
    ...
    if (output_queue_size > 0)
    {
      ...
      cv::Mat m_out_visualization_image =
          generateVisualizationImage(new_frame->vcam_depth_image_floor_pixels_removed_8bpp_,
      ...
      publishImageAsRosMsg(m_out_visualization_image, "bgr8", "None", out_image_publisher_,
                           enable_output_image_compression_);
      PROFILE_FUNCTION_START(Publish_CompressImg)
      ...
      {
        output_sensor_->write(new_frame->frame_number_, new_frame->object_detected_, new_frame->depth_frame_with_floor_,
