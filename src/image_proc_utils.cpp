...
void ImageProcUtils::convertTo8BppImage(ADIImage * in_img, ADIImage * out_img, int scale_factor)
{
  if ((in_img != nullptr) && (out_img != nullptr)) {
    if (
      (in_img->data != nullptr) && (out_img->data != nullptr) && (in_img->bpp == 16) &&
      (out_img->bpp == 8)) {
...
void ImageProcUtils::transformFrame(
  ...
  CameraExtrinsics * dst_extrinsics, ADIImageROI * out_roi, bool /*compute_point_cloud_enable*/,
  short * xyz_frame)
{
    ...
    if ((in_img->data != nullptr) && (out_img->data != nullptr)) {
      ...
      for (int i = 0; i < img_height; i++) {
        for (int j = 0; j < img_width; j++) {
          ...
          float dstc_coord[3];
          dstc_coord[0] =
            (rot_matrix[0] * src_xc_mm + rot_matrix[1] * src_yc_mm + rot_matrix[2] * src_zc_mm) +
            trans_matrix[0];
          dstc_coord[1] =
            (rot_matrix[3] * src_xc_mm + rot_matrix[4] * src_yc_mm + rot_matrix[5] * src_zc_mm) +
            trans_matrix[1];
          dstc_coord[2] =
            (rot_matrix[6] * src_xc_mm + rot_matrix[7] * src_yc_mm + rot_matrix[8] * src_zc_mm) +
            trans_matrix[2];
